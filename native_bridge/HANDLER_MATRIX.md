...
| `create_object` | `handle_create_object` | write | main/plugin event context | 1 | Creates and then deletes a test rectangle; creates/selects `Vectorworks MCP Layer` only when no writable layer exists |
| `batch_create_objects` | `handle_batch_create_objects` | write | main/plugin event context | 1 | Atomically creates multiple phase-1 primitives in one undo event; uses the same no-layer bootstrap as `create_object` |
