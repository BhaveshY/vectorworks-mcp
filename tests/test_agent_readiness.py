...
class AgentReadinessTests(unittest.TestCase):
    ...
    def test_agent_instruction_files_exist(self):
        ...
        self.assertIn("-FullNative -Json", codex)
        ...
        self.assertIn("-FullNative", agents)
        ...
        self.assertIn("-FullNative -Json", agent_install)
        ...
        self.assertIn("native_summary.next_stage", agent_install)
    ...
    def test_root_install_script_is_agent_parseable_and_host_only_by_default(self):
        ...
        self.assertIn("Python.Python.3.12", install)
        self.assertIn("Install-WingetPackage", install)
        self.assertIn("Invoke-ProcessCapture", install)
        self.assertIn("RedirectStandardOutput", install)
        self.assertIn("RedirectStandardError", install)
        self.assertIn("ReadToEndAsync", install)
        ...
        self.assertIn("dependency_checks", install)
        self.assertIn("production_ready", install)
        ...
        self.assertNotIn("@Arguments 2>&1", install)
        self.assertNotIn("@Args 2>&1", install)
...
""",
    ...
    def test_native_bridge_source_scaffold_encodes_threading_contract(self):
        ...
        self.assertIn("StartMainContextPump", combined)
        self.assertIn("StopMainContextPump", combined)
        self.assertIn("SetTimer", combined)
        self.assertIn("CadHandlersRuntimeReady", combined)
        self.assertIn("main_context_pump_ready", combined)
...
""",
            ...
            self.assertIn("Ws2_32.lib;User32.lib;%(AdditionalDependencies)", project_text)
            ...
            self.assertEqual(project.read_text(encoding="utf-8").count("User32.lib"), 1)
...
""".strip(),
    ...
    def test_native_smoke_script_is_documented(self):
        ...
        self.assertIn("ReusedExistingBridge", start_smoke_script)
        self.assertIn("InitialSmokeAttempted", start_smoke_script)
        self.assertIn("Invoke-NativeSmokeAttempt", start_smoke_script)
        self.assertIn("[Math]::Min(10, $StartupTimeoutSeconds)", start_smoke_script)
        ...
        self.assertIn('main_context_pump: "win32_ui_timer"', native_readme)
        self.assertIn("main_context_pump_ready: true", native_readme)
        ...
        self.assertIn('main_context_pump: "win32_ui_timer"', protocol)
        self.assertIn("main_context_pump_ready: true", protocol)
