...
class NativeAuthoritativeCapabilitiesContractTests(unittest.TestCase):
    ...
    def test_view_and_document_readback_model_semantic_truth(self):
        ...
        zero_layer_open = source.index("if (!gSDK->GetActiveLayer())")
        open_file_scan = source.index("gSDK->GetOpenFilesList(openFiles)")
        sdk_document_open = source.index("gSDK->OpenDocumentPath(identifier, false)")
        self.assertLess(zero_layer_open, open_file_scan)
        self.assertIn("IExtendedPropsPtr", source[zero_layer_open:open_file_scan])
        self.assertIn("OpenOrActivateFile", source[zero_layer_open:open_file_scan])
        self.assertGreater(sdk_document_open, open_file_scan)
