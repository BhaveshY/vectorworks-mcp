...
- Native bridge source/build state is unclear: run `${CLAUDE_PLUGIN_ROOT}\scripts\doctor-native-bridge.ps1 -Json`, then follow `nextCommand` and explain `nextCommandReason`; use `nextCommandSpec.executable` and `nextCommandSpec.arguments` for programmatic execution.
