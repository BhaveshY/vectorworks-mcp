...
- Native bridge source worktree exists but has no bridge scaffold: run `${CLAUDE_PLUGIN_ROOT}\scripts\copy-native-bridge-scaffold.ps1` after the unmodified SDK example builds.
...
- `vw_ping` returns `cad_api_safe: false` or `transport_only: true`: do not call CAD handlers. Regenerate/copy/run the stable loader for today's workflow, or run the companion repo's native bridge prerequisite checker if the user is explicitly building the long-term SDK bridge.
