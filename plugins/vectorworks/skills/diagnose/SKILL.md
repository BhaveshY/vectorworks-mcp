...
- CAD tool returns `blocked: true`: the host-side safety guard prevented a real CAD call. Follow its `reason` and `next_action`; usually regenerate/run the dialog launcher or switch to a compiled native SDK bridge.
...
- `vw_ping` returns `cad_api_safe: false` or `transport_only: true`: do not call CAD handlers. Regenerate the dialog launcher for today's workflow, or run the companion repo's native bridge prerequisite checker if the user is explicitly building the long-term SDK bridge.
