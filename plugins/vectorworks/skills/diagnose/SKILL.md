...
- Native bridge source/build state is unclear: run `${CLAUDE_PLUGIN_ROOT}\scripts\invoke-native-bridge-next.ps1 -Json` first so safety flags are enforced. Inspect `status`, `missingAllowFlags`, `safetyBlocks`, `validationErrors`, and `sdkArchiveCandidates`; reuse downloaded SDK ZIPs with `-SdkArchivePath`. `invalid_spec` is a hard stop. Use `${CLAUDE_PLUGIN_ROOT}\scripts\doctor-native-bridge.ps1 -Json` only for lower-level inspection of `nextCommand`, `nextCommandReason`, and `nextCommandSpec`.
